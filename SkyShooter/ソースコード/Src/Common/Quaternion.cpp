...
Quaternion Quaternion::Euler(double radX, double radY, double radZ)
{
    ...
    radX = MathUtility::RadIn2PI(radX);
    radY = MathUtility::RadIn2PI(radY);
    radZ = MathUtility::RadIn2PI(radZ);
...
{
    dir = MathUtility::VNormalize(dir);
    VECTOR right = MathUtility::VNormalize(VCross(up, dir));
    up = VCross(dir, right);
...
Quaternion Quaternion::FromToRotation(VECTOR fromDir, VECTOR toDir)
{
	...
	double angle = MathUtility::AngleDeg(fromDir, toDir);
	if (angle >= 179.9196)
	{
		auto r = VCross(fromDir, MathUtility::DIR_R);
		axis = VCross(r, fromDir);
		//if (axis.sqrMagnitude < 0.000001f)
		float len = axis.x * axis.x + axis.y * axis.y + axis.z * axis.z;
		if (len < 0.000001f)
		{
			axis = MathUtility::DIR_U;
		}
	}

	axis = MathUtility::VNormalize(axis);
	return Quaternion::AngleAxis(MathUtility::Deg2RadD(angle), axis);
}
